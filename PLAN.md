...
- **狀態**：✅ 已完成（2026-05-12）
- **實作**：Banner 新增「字級 小／中／大」切換鈕，預設「中」(16px)；小=14px、大=20px；以 `document.documentElement.style.fontSize` 控制，`localStorage` 持久化。手機版按鈕觸控區 44px，隱藏隱私提示以節省空間。

#### UX-2｜首次進入引導（Onboarding）
- **問題**：新使用者不知道這頁在幹嗎，需要開發者解說
- **方向**：首次進入顯示一個簡短說明彈窗或橫幅（用途 + 3 步驟流程），可關閉，關閉狀態存 `localStorage`
- **狀態**：✅ 已完成（2026-05-12）
- **實作**：全螢幕 Modal，顯示三步驟說明（填寫→預覽→列印）＋隱私保證＋開發者 credit。按「開始填寫」或 Escape 關閉，寫入 `localStorage('safecard-onboarded')`，之後不再顯示。
...
- **狀態**：✅ 已完成（2026-05-12）
- **實作（視覺對照）**：每張卡預覽下方新增等比例小方塊圖示（橘=本卡 / 灰=悠遊卡），搭配尺寸文字說明（避難卡「與悠遊卡相同大小」；醫療/寵物卡「約明信片大小」）。
- **實作（實際尺寸預覽）**：預覽區右上角新增「📏 實際尺寸預覽」按鈕，點擊後跳出校正 Modal——顯示 85.6×54mm 虛線框，引導使用者拿悠遊卡對準螢幕、調整瀏覽器縮放至吻合，確認後將 `--hs`/`--ps`/`--ms` 設為 1，以 1:1 顯示卡片。手機版隱藏此功能。

#### 開發者 Credit 與授權
- **狀態**：✅ 已完成（2026-05-12）
- **實作**：Footer 顯示「© 2025 Claire Cheng · GitHub · MIT License」；Onboarding Modal 底部顯示「由 Claire Cheng 開發 · GitHub · MIT 授權」，兩者皆連結至 [claire-cheng.com/zh](https://claire-cheng.com/zh) 及 [GitHub repo](https://github.com/Legendream/safecard)。授權採 MIT。

#### 寵物卡手冊引導
- **狀態**：✅ 已完成（2026-05-12）
- **實作**：寵物卡 tab 表單最上方新增橘色提示框，引導使用者搭配《動物夥伴防災避難手冊》使用，附 Google Drive 連結。
...
#### FEAT-1｜藥袋拍照自動辨識用藥
- **功能**：使用者拍攝藥袋後，自動辨識藥名、劑量、頻率，填入「目前用藥」欄位（醫療卡 `m_medication` 或寵物卡用藥欄）
- **評估方向**：
